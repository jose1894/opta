...
<template>
  <CardBox isForm @submit.prevent="submit">
    <div 
      :class="path !== 'create' ? 'grid md:grid-cols-3 gap-4' : 
      'grid md:grid-cols-2 gap-4'">
      <FormField 
        v-show="path !== 'create'"
        :label="$t('message.currencies.code')">
        <FormControl :name="'codigo'" v-model="currency.codigo" :icon="mdiCodeBraces" readonly="true"/>            
      </FormField>
