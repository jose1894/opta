...
<template>
  <LayoutAuthenticated>
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiGlobeModel" :title="$t('message.industry.industries')">
        <BaseButton :to="{name: 'IndustriesCreate'}" :icon="mdiPlus" :label="$t('message.add_new')" color="success" small />
      ...
      <CardBox v-if="mainStore?.industries?.industrias?.length" class="mb-6" has-table>
        <IndustryTable :checkDelete="customCheckDelete" @changePage="onChangePage" @sort="onSortPage" />
      </CardBox>
