...
const action = (industry) =>{
  const {_id, codigo, nombre, estado } = industry.value;
  const data = {_id, codigo, nombre, estado: estado.id}
  if (props.path === 'create'){
    return industriesService.create(data)
  } 

  return industriesService.update(data);
}

onMounted(async () => {
  if (props.path === 'update'){
    const res = await industriesService.read(route.params);
    industryData.value = res.data
    industryData.value.estado = selectOptions.filter(status => status.id === res.data.estado)[0]
  }
})
...
const v$ = useValidate(rules, industryData);

const successMessage = props.path === 'create' ? t("message.industry.created.success") : t("message.industry.updated.success")
const errorMessage = props.path === 'create' ? t("message.industry.created.error") : t("message.industry.updated.error")
...
const goTo = () => router.push('/setup/industries')
</script>
<template>
  <CardBox isForm @submit.prevent="submit">
    <div :class="path !== 'create' ? 'grid md:grid-cols-3 gap-3' : 
      ...
      <FormField :label="$t('message.industry.status')" :help="v$?.estado?.$errors[0]?.$message">
        <FormControl v-model="industryData.estado" :icon="mdiListStatus" :options="selectOptions" />
      </FormField>
    </div>
    <template #footer>      
