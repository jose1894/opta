...
const activateItem = () => {
  const { _id } = selectedProfessions.value
  professionsService.restore(_id).then(() => {
      toast.success(t("message.profession.restore.success"));
      emit('changePage', currentPage.value)      
    })
    .catch(err => {
      toast.error(`${t("message.profession.restore.error")} ${err?.response?.data.msg}`)
    });
}
</script>
...
<CardBoxModal 
  ...
  <CardBoxModal 
    ...
    <strong>{{ $t('message.profession.restore.question') }} <b> {{ dataName() }} </b></strong> ?   
  </CardBoxModal>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th @click="sort('nombre')">{{ $t('message.profession.name') }}</th>
      ...
      <tr v-for="(profession, index) in itemsPaginated" 
        ...
        <td class="before:hidden lg:w-1 whitespace-nowrap">
          <BaseButtons type="justify-start lg:justify-end" no-wrap>
            <BaseButton
              v-show="checkDelete && profession.estado === 2"
              color="success"
              :icon="mdiRestore"
              small
            ...
            <BaseButton v-show="!checkDelete && profession.estado !== 2" color="danger" :icon="mdiTrashCan" small @click="isModalDangerActive = true" />
          </BaseButtons>
        </td>
      </tr>
    </tbody>
