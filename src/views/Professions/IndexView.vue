...
import ProfessionsTable from './ProfessionsTable.vue';
...
const getProfessionsDelete = (data) => {
    professionsService.getDelete(data).then(response => {
      mainStore.professions = response
      page.value = response.page
      perPage.value = response.perPage
    })
...
<template>
  <LayoutAuthenticated>
    <SectionMain>
      ...
      <SectionTitleLineWithButton v-if="!mainStore?.professions" :icon="mdiTableOff" title="Empty variation" />

      <NotificationBar v-if="!mainStore?.professions" color="danger" :icon="mdiTableOff">
