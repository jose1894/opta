...
const activateItem = () => {
  const { _id } = selectedCategorias.value
  categoriesService.restore(_id).then(() => {
      toast.success(t("message.category.restore.success"));
      emit('changePage', currentPage.value)      
    })
    .catch(err => {
      toast.error(`${t("message.category.restore.error")} ${err?.response?.data.msg}`)
    });
}
</script>
...
<CardBoxModal 
  ...
  <CardBoxModal 
    ...
    <strong>{{ $t('message.category.restore.question') }} <b> {{ dataName() }} </b></strong> ?   
  </CardBoxModal>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th @click="sort('nombre')">{{ $t('message.category.name') }}</th>
      ...
      <tr v-for="(category, index) in itemsPaginated" 
        ...
        <td class="before:hidden lg:w-1 whitespace-nowrap">
          <BaseButtons type="justify-start lg:justify-end" no-wrap>
            <BaseButton
              v-show="checkDelete && category.estado === 2"
              color="success"
              :icon="mdiRestore"
              small
            ...
            <BaseButton v-show="!checkDelete && category.estado !== 2" color="danger" :icon="mdiTrashCan" small @click="isModalDangerActive = true" />
